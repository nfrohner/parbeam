...
struct AuxiliaryData
    ...
    bounds_by_state::Union{Array{UInt32, 4}, Array{UInt32, 5}}
end
...
function initialize_beam(configuration::Configuration, instance::Instance)
    ...
    games = Array{UInt16, 2}(undef, configuration.beam_width, number_of_games(instance))
    forbidden_opponents = Array{UInt8, 2}(undef, configuration.beam_width, instance.n)
    rounds = Array{UInt8, 2}(undef, configuration.beam_width, instance.n)
    positions = Array{UInt8, 2}(undef, configuration.beam_width, instance.n)
    possible_away_streaks = Array{UInt8, 2}(undef, configuration.beam_width, instance.n)
    possible_home_stands = Array{UInt8, 2}(undef, configuration.beam_width, instance.n)
    heuristic_estimates = Array{Int, 2}(undef, configuration.beam_width, instance.n)
    number_of_away_games_left = Array{UInt8, 2}(undef, configuration.beam_width, instance.n)
    number_of_home_games_left = Array{UInt8, 2}(undef, configuration.beam_width, instance.n)
    away_teams_left_masks = Array{UInt32, 2}(undef, configuration.beam_width, instance.n)
...
function initialize_root(instance::Instance, Q::TTPBeam)
    ...
    for i in 1:number_of_games(instance)
        Q.additional_data.games[1,i] = i
    end

    #prepare_auxiliary_data(instance, Q, 1, 0)

    for i in 1:instance.n
        Q.additional_data.forbidden_opponents[1,i] = 0
        Q.additional_data.rounds[1,i] = 0
        Q.additional_data.positions[1,i] = i
        Q.additional_data.possible_away_streaks[1,i] = instance.streak_limit
        Q.additional_data.possible_home_stands[1,i] = instance.streak_limit
        Q.additional_data.heuristic_estimates[1,i] = TTPGuidance.heuristic_estimate(instance.streak_limit, instance.d, i, 2^(instance.n-1) - 1, instance.n-1, instance.n-1, i, 0, instance.aux_by_thread[Threads.threadid()].bounds_by_state)
        Q.additional_data.number_of_away_games_left[1,i] = instance.n-1
        Q.additional_data.number_of_home_games_left[1,i] = instance.n-1
        Q.additional_data.away_teams_left_masks[1,i] = 2^(instance.n - 1) -1
    end
end

function initial_lb(configuration::Configuration, instance::Instance, beam::TTPBeam)
    heuristics_sum = 0
    for i in 1:instance.n
...
function next_team_dynamic(instance::Instance, beam::TTPBeam, Q_idx::Int, auxiliary_data::AuxiliaryData, min_round::Int)
    ...
    for game_idx in (layer+1):number_of_games(instance)
        j = beam.additional_data.games[Q_idx, game_idx]
        game = instance.games[j]
        heuristic_estimates_delta_away_team = 0
        heuristic_estimates_delta_home_team = 0
        if beam.additional_data.rounds[Q_idx,game[1]] == min_round && game_allowed(instance, beam.additional_data, Q_idx, game[1], game[2]) && delta_feasibility_check(instance, beam.additional_data, Q_idx, game[1], game[2])
...
function calc_heuristic_estimates_delta(instance::Instance, additional_data::AdditionalData, Q_idx::Int, away_team::Int, home_team::Int, auxiliary_data::AuxiliaryData)
    heuristic_estimate_delta_away_team = -additional_data.heuristic_estimates[Q_idx,away_team]
    streak = instance.streak_limit - (additional_data.possible_away_streaks[Q_idx,away_team] - 1)
    if home_team < away_team
        teams_left_mask = additional_data.away_teams_left_masks[Q_idx,away_team] - 2 ^ (home_team-1)
    else
        teams_left_mask = additional_data.away_teams_left_masks[Q_idx,away_team] - 2 ^ (home_team-2)
    end

    heuristic_estimate_delta_away_team += TTPGuidance.heuristic_estimate(instance.streak_limit, instance.d, away_team, teams_left_mask*1, additional_data.number_of_away_games_left[Q_idx,away_team]-1, additional_data.number_of_home_games_left[Q_idx,away_team]+0, home_team, streak, auxiliary_data.bounds_by_state)
    
    heuristic_estimate_delta_home_team = -additional_data.heuristic_estimates[Q_idx,home_team]
    streak = instance.streak_limit - (additional_data.possible_home_stands[Q_idx,home_team] - 1)
    teams_left_mask = additional_data.away_teams_left_masks[Q_idx,home_team]
    heuristic_estimate_delta_home_team += TTPGuidance.heuristic_estimate(instance.streak_limit, instance.d, home_team, teams_left_mask*1, additional_data.number_of_away_games_left[Q_idx,home_team]+0, additional_data.number_of_home_games_left[Q_idx,home_team]-1, home_team, streak, auxiliary_data.bounds_by_state)
...
function next_team(instance::Instance, additional_data::AdditionalData, Q_idx::Int)
    ...
    min_round = additional_data.rounds[Q_idx,min_team]
    max_round = additional_data.rounds[Q_idx,min_team]
    for i in 2:instance.n
        team = instance.teams_permutation[i]
        if additional_data.rounds[Q_idx,team] < min_round
            min_team = team
            min_round -= 1
            break
        end
...
function game_allowed(instance::Instance, additional_data::AdditionalData, Q_idx::Int, away_team::Int, home_team::Int)
    additional_data.rounds[Q_idx,away_team] == additional_data.rounds[Q_idx,home_team] && additional_data.possible_away_streaks[Q_idx,away_team] > 0 && additional_data.possible_home_stands[Q_idx,home_team] > 0 && (!instance.no_repeat || (additional_data.forbidden_opponents[Q_idx,away_team] != home_team && additional_data.forbidden_opponents[Q_idx,home_team] != away_team))
end

function travel_distance_after_next_game(instance::Instance, beam::TTPBeam, Q_idx::Int, away_team::Int, home_team::Int)
    beam.nodes.costs_so_far[Q_idx] + instance.d[beam.additional_data.positions[Q_idx,away_team],home_team] + instance.d[beam.additional_data.positions[Q_idx,home_team],home_team]
end
...
function terminal_costs(instance::Instance, beam::TTPBeam, Q_idx::Int)
    ...
    for i in 1:instance.n
        remaining_terminal_costs += instance.d[beam.additional_data.positions[Q_idx,i], i]
    end
...
function copy(instance::Instance, from_Q::TTPBeam, to_Q::TTPBeam, from_idx::Int, to_idx::Int)
    ...
    for i in 1:number_of_games(instance)
        to_Q.additional_data.games[to_idx,i] = from_Q.additional_data.games[from_idx,i]
    end
    for i in 1:instance.n
        to_Q.additional_data.rounds[to_idx,i] = from_Q.additional_data.rounds[from_idx,i]
        to_Q.additional_data.forbidden_opponents[to_idx,i] = from_Q.additional_data.forbidden_opponents[from_idx,i]
        to_Q.additional_data.positions[to_idx,i] = from_Q.additional_data.positions[from_idx,i]
        to_Q.additional_data.possible_away_streaks[to_idx,i] = from_Q.additional_data.possible_away_streaks[from_idx,i]
        to_Q.additional_data.possible_home_stands[to_idx,i] = from_Q.additional_data.possible_home_stands[from_idx,i]
        to_Q.additional_data.heuristic_estimates[to_idx,i] = from_Q.additional_data.heuristic_estimates[from_idx,i]
        to_Q.additional_data.number_of_away_games_left[to_idx,i] = from_Q.additional_data.number_of_away_games_left[from_idx,i]
        to_Q.additional_data.number_of_home_games_left[to_idx,i] = from_Q.additional_data.number_of_home_games_left[from_idx,i]
        to_Q.additional_data.away_teams_left_masks[to_idx,i] = from_Q.additional_data.away_teams_left_masks[from_idx,i]
    end
end
...
function make_transition(instance::Instance, successor_specs::TTPSuccessorData, Q::TTPBeam, Q_idx::Int, spec_idx::Int)
...
function make_move(instance::Instance, Q::TTPBeam, Q_idx::Int, next_j::UInt16, heuristic_estimates_delta_away_team::Int, heuristic_estimates_delta_home_team::Int)
    ...
    addional_data_after_next_game!(instance, Q, Q_idx, next_game[1], next_game[2], heuristic_estimates_delta_away_team, heuristic_estimates_delta_home_team)
    layer = Q.nodes.layer[Q_idx]
    j_to_swap = Q.additional_data.games[Q_idx,layer]
    ...
    if next_j != j_to_swap
        ...
        for j_idx in (layer+1):number_of_games(instance)
            if Q.additional_data.games[Q_idx,j_idx] == next_j
                target_idx = j_idx
                break 
            end
        end
        @assert target_idx != -1
        Q.additional_data.games[Q_idx,layer], Q.additional_data.games[Q_idx,target_idx] = Q.additional_data.games[Q_idx,target_idx], Q.additional_data.games[Q_idx,layer]
    end
end

function addional_data_after_next_game!(instance::Instance, Q::TTPBeam, idx::Int, away_team::Int, home_team::Int, heuristic_estimates_delta_away_team::Int, heuristic_estimates_delta_home_team::Int)
    #print_state_and_game(instance, Q, idx, away_team, home_team)
    # heuristic estimates delta
    Q.additional_data.heuristic_estimates[idx,away_team] += heuristic_estimates_delta_away_team
    Q.additional_data.heuristic_estimates[idx,home_team] += heuristic_estimates_delta_home_team

    # rounds
    Q.additional_data.rounds[idx,away_team] += 1
    Q.additional_data.rounds[idx,home_team] += 1
        
    # streaks
    max_away_streak_away_team = min(Q.additional_data.number_of_away_games_left[idx,away_team], instance.streak_limit)
    max_home_stand_away_team = min(Q.additional_data.number_of_home_games_left[idx,away_team], instance.streak_limit)
    max_away_streak_home_team = min(Q.additional_data.number_of_away_games_left[idx,home_team], instance.streak_limit)
    max_home_stand_home_team = min(Q.additional_data.number_of_home_games_left[idx,home_team], instance.streak_limit)

    if Q.additional_data.positions[idx,away_team] == away_team
        Q.additional_data.possible_away_streaks[idx,away_team] = max_away_streak_away_team - 1
        Q.additional_data.possible_home_stands[idx,away_team] = max_home_stand_away_team
    else
        Q.additional_data.possible_away_streaks[idx,away_team] -= 1
        Q.additional_data.possible_home_stands[idx,away_team] = max_home_stand_away_team
    end

    if Q.additional_data.positions[idx,home_team] == home_team
        Q.additional_data.possible_away_streaks[idx,home_team] = max_away_streak_home_team
        Q.additional_data.possible_home_stands[idx,home_team] -= 1
    else
        Q.additional_data.possible_away_streaks[idx,home_team] = max_away_streak_home_team
        Q.additional_data.possible_home_stands[idx,home_team] = max_home_stand_home_team - 1
    end

    # number of games left
    Q.additional_data.number_of_away_games_left[idx,away_team] -= 1
    Q.additional_data.number_of_home_games_left[idx,home_team] -= 1

    repeater_exists = false
    if home_team < away_team
        if Q.additional_data.away_teams_left_masks[idx,home_team] & 2 ^ (away_team-2) > 0
            repeater_exists = true
        end
        Q.additional_data.away_teams_left_masks[idx,away_team] -= 2 ^ (home_team-1)
    else
        if Q.additional_data.away_teams_left_masks[idx,home_team] & 2 ^ (away_team-1) > 0
            repeater_exists = true
        end
        Q.additional_data.away_teams_left_masks[idx,away_team] -= 2 ^ (home_team-2)
    end

    # positions
    Q.additional_data.positions[idx,away_team] = home_team
    Q.additional_data.positions[idx,home_team] = home_team

    # forbidden opponents
    if repeater_exists
        Q.additional_data.forbidden_opponents[idx,away_team] = home_team
        Q.additional_data.forbidden_opponents[idx,home_team] = away_team
    else
        Q.additional_data.forbidden_opponents[idx,away_team] = 0
        Q.additional_data.forbidden_opponents[idx,home_team] = 0
    end

    for team in 1:instance.n
        if team != away_team && team != home_team && (Q.additional_data.forbidden_opponents[idx,team] == away_team || Q.additional_data.forbidden_opponents[idx,team] == home_team)
            Q.additional_data.forbidden_opponents[idx,team] = 0
        end
    end

    @assert 0 <= Q.additional_data.possible_away_streaks[idx,away_team] <= instance.streak_limit
    @assert 0 <= Q.additional_data.possible_away_streaks[idx,home_team] <= instance.streak_limit
    @assert 0 <= Q.additional_data.possible_home_stands[idx,away_team] <= instance.streak_limit
    @assert 0 <= Q.additional_data.possible_home_stands[idx,home_team] <= instance.streak_limit
end
...
function delta_feasibility_check(instance::Instance, additional_data::AdditionalData, Q_idx::Int, away_team::Int, home_team::Int)
    away_team_home_games_left = additional_data.number_of_home_games_left[Q_idx,away_team]*1
    away_team_away_games_left = additional_data.number_of_away_games_left[Q_idx,away_team] - 1
    home_team_home_games_left = additional_data.number_of_home_games_left[Q_idx,home_team] - 1
    home_team_away_games_left = additional_data.number_of_away_games_left[Q_idx,home_team]*1
...
function dead_team_check(instance::Instance, beam::TTPBeam, Q_idx::Int, min_round::Number, max_round::Number)
    for i in 1:instance.n
        if (min_round == max_round || beam.additional_data.rounds[Q_idx,i] < max_round) && instance.aux_by_thread[Threads.threadid()].number_of_permitted_games_left[i] == 0 && instance.aux_by_thread[Threads.threadid()].number_of_home_games_left[i] + instance.aux_by_thread[Threads.threadid()].number_of_away_games_left[i] > 0
            return false
        end
    end
...
function dead_team_check_without_aux(instance::Instance, beam::TTPBeam, Q_idx::Int, layer::Number, min_round::Number, max_round::Number)
    teams_mask = 0
    for i in 1:instance.n
        if beam.additional_data.number_of_home_games_left[Q_idx,i] + beam.additional_data.number_of_away_games_left[Q_idx,i] == 0 || (min_round != max_round && beam.additional_data.rounds[Q_idx,i] == max_round)
            teams_mask |= 2^(i-1)
        end
    end
    for game_idx in (layer+1):number_of_games(instance)
...
function successors_size(successor_specs::TTPSuccessorData)
    (sizeof(successor_specs.layer)+sizeof(successor_specs.costs_after_move)+sizeof(successor_specs.priority)+sizeof(successor_specs.Q_idx)+sizeof(successor_specs.Q_idx)+sizeof(successor_specs.successor_move)+sizeof(successor_specs.additional_data.heuristic_estimates_delta_away_team)+sizeof(successor_specs.additional_data.heuristic_estimates_delta_home_team))/(1024^3)
end
